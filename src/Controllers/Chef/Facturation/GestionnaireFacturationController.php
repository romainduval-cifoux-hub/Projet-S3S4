...
class GestionnaireFacturationController
{
    ...
    private function buildPdf(FPDF $pdf, array $facture, array $entreprise): void
    {
        $tvaTaux = 0.20;
        $vert = [77, 110, 55];
    ...
    public function handleRequest()
    {
        ...
        if (!empty($_POST['idDoc']) && in_array($action, ['payer', 'accepter', 'refuser', 'send'], true)) {
            $idDoc = (int)$_POST['idDoc'];
            ...
            if ($action === 'send') {
                ...
                // ⚠️ Il faut récupérer l'email du client via une jointure (Document->clients->users)
                // Dans getFactureById(), retourne u.username AS emailClient
                $emailClient = $doc['emailClient'] ?? '';
                if ($emailClient === '') {
                    http_response_code(400);
                    exit("Email du client introuvable (client non lié à un user).");
                }

                $entreprise = getEntrepriseInfo($this->pdo);

                // Générer PDF vers un fichier temporaire
                $numero = $doc['num'] ?? $idDoc;
                $tmpPdf = sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'document_' . $numero . '_' . uniqid() . '.pdf';

                $pdf = new FPDF('P', 'mm', 'A4');
                $this->buildPdf($pdf, $doc, $entreprise);

                while (ob_get_level() > 0) ob_end_clean();
                $pdf->Output('F', $tmpPdf);

                $subject = "Votre " . ($doc['typeDoc'] ?? 'document') . " - Team Jardin";
                $text = "Bonjour,\n\nVeuillez trouver en pièce jointe votre document.\n\nCordialement,\nTeam Jardin";

                $ok = sendMailViaMailgunWithPdf($emailClient, $subject, $text, $tmpPdf);
                ...
                if ($ok) {
                    enregistrerRelance($this->pdo, $idDoc);
                    $_SESSION['factu_success'] = "Document envoyé à " . ($emailClient ?? 'le client') . ".";
                } else {
                    $_SESSION['factu_error'] = "Échec de l’envoi du document.";
                }


                header('Location: ' . BASE_URL . '/public/index.php?page=chef/facturation/GestionFacturation');
                exit;
            }
